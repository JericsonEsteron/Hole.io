...
namespace Community
{
    ...
    public class ReplaceWithPrefab : EditorWindow
    {
        ...
        /// </summary>
        [MenuItem("Tools/Community/Replace with Prefabaaaaaa")]
        ...
        void OnGUI()
        {
            ...
            {
                editMode = GUILayout.Toggle(editMode, new GUIContent("Edit", "Start using this feature"), EditorStyles.toggle);
            ...
            keepOriginalNames = GUILayout.Toggle(keepOriginalNames, "Keep names", EditorStyles.toggle);
            applyRotation = GUILayout.Toggle(applyRotation, "Apply rotation", EditorStyles.toggle);
            applyScale = GUILayout.Toggle(applyScale, "Apply scale", EditorStyles.toggle);
            keepPlaceInHierarchy = GUILayout.Toggle(keepPlaceInHierarchy, "Keep Place In Hierarchy", EditorStyles.toggle);

            #region "TAG LAYER"

            SearchWithTag = GUILayout.Toggle(SearchWithTag, "Apply Search By Tag", EditorStyles.toggle);
            SearchWithLayer = GUILayout.Toggle(SearchWithLayer, "Apply Search By Layer");
            //if(SearchWithTag)
            //{
            //    TagForSearch = EditorGUILayout.TagField(TagForSearch);
            //}
        ...
        void Replace(GameObject obj)
        {
            ...
            newObject.gameObject.tag = obj.tag;
            newObject.gameObject.layer = obj.layer;

            if (obj.GetComponent<MeshRenderer>())
            {
                if (newObject.GetComponent<MeshRenderer>())
                {
                    if (obj.GetComponent<MeshRenderer>().sharedMaterials.Length == newObject.GetComponent<MeshRenderer>().sharedMaterials.Length)
                    {
                        Material[] CacheMaterials = new Material[obj.GetComponent<MeshRenderer>().sharedMaterials.Length];
                        for (int a = 0; a < obj.GetComponent<MeshRenderer>().sharedMaterials.Length; a++)
                        {
                            CacheMaterials[a] = obj.GetComponent<MeshRenderer>().sharedMaterials[a];
                        }
                        for (int b = 0; b < CacheMaterials.Length; b++)
                        {
                            newObject.GetComponent<MeshRenderer>().sharedMaterials[b] = CacheMaterials[b];
                        }
            ...
            {
                if (newObject.GetComponent<SkinnedMeshRenderer>())
                {
                    if (obj.GetComponent<SkinnedMeshRenderer>().sharedMaterials.Length == newObject.GetComponent<SkinnedMeshRenderer>().sharedMaterials.Length)
                    {
                        Material[] CacheMaterials = new Material[obj.GetComponent<SkinnedMeshRenderer>().sharedMaterials.Length];
                        for (int a = 0; a < obj.GetComponent<SkinnedMeshRenderer>().sharedMaterials.Length; a++)
                        {
                            CacheMaterials[a] = obj.GetComponent<SkinnedMeshRenderer>().sharedMaterials[a];
                        }
                        for (int b = 0; b < CacheMaterials.Length; b++)
                        {
                            newObject.GetComponent<SkinnedMeshRenderer>().sharedMaterials[b] = CacheMaterials[b];
                        }
